...
export default function expenseCategoryPage() {
      ...
      try {
        ...
        if (response.resultCode === 1008) {
          const expenseCategories = response.data.filter(
            (category) => category.transactiontypeid === 2
          );
  ...
  const handleRegisterCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sbody = {
        ...
        transactiontype: 2, // expense type
      };
  ...
  if (loading) return <p className="text-center text-xl">Төрөл уншиж байна...</p>;

  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="max-w-lg w-full bg-white shadow-lg rounded-lg p-6">
        <h2 className="text-2xl font-bold text-center mb-6">Зарлагын төрлүүд</h2>
        ...
        <form onSubmit={handleRegisterCategory} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="төрлийн нэрээ оруулна уу"
            />
          </div>
        ...
        <p className="mt-4 text-sm text-gray-500 text-center">
          Буцах{" "}
          <a href="/addtransaction?transactiontypeid=2" className="text-indigo-500 hover:underline">
          Гүйлгээ нэмэх
          </a>
        </p>
      </div>
